...
var middleware = function(app, mdb){
    ...
    api.add({   "url": urlJoin("/api", "post", "find-user"),
                "param": [
            ...
            function(req, res, obj){
                ...
                global.app.console.log(msg, req.body);

                if (req.body.id == 'empty'){
                    api.response(res, false, false, obj);
                } else {
                    var ObjectId = mongoose.Types.ObjectId;
                    mdb.models.users.findOne({ '_id': req.body.id }, 'name email', function (err, person) {
    ...
    api.add({   "url": urlJoin("/api", "get", "session", "valid"),
            ...
            function(req, res, obj){
                ...
                mdb.models.session.findOne({ 'hash': req.cookies[ini.cookie.ts.session] }, 'user live', function (err, session) { // <-- error is here because there is no session
